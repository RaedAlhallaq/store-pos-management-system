...
const fmt = (n: number | string) =>
  parseFloat(String(n || 0)).toLocaleString('ar-SA', { minimumFractionDigits: 2, maximumFractionDigits: 2 });
...
export const DashboardPage: React.FC = () => {
  ...
  const { data: todayReport } = useQuery({
    queryKey: ['today-report', todayStr],
    queryFn: () => reportsApi.getProfitLoss({ date_from: todayStr, date_to: todayStr }),
  });
  ...
  const { data: customersData } = useQuery({
    ...
    queryFn: () => customersApi.getCustomers({ per_page: 200 }),
  });

  const { data: suppliersData } = useQuery({
    queryKey: ['suppliers-summary'],
    queryFn: () => suppliersApi.getSuppliers({ per_page: 200 }),
  });

  const { data: todayPurchases } = useQuery({
    queryKey: ['today-purchases', todayStr],
    queryFn: () => purchasesApi.getPurchases({ date_from: todayStr, date_to: todayStr, per_page: 200 }),
  });

  const { data: todayExpenses } = useQuery({
    queryKey: ['today-expenses', todayStr],
    queryFn: () => expensesApi.getExpenses({ date_from: todayStr, date_to: todayStr, per_page: 200 }),
  });

  const totalCustomerDebt =
    customersData?.data?.reduce((sum, c) => sum + parseFloat(c.current_balance || '0'), 0) ?? 0;

  const totalSupplierPayables =
    suppliersData?.data?.reduce((sum, s) => sum + parseFloat(s.current_balance || '0'), 0) ?? 0;

  const todayPurchasesTotal =
    todayPurchases?.data?.reduce((sum, p) => sum + parseFloat(p.grand_total || '0'), 0) ?? 0;

  const todayExpensesTotal =
    todayExpenses?.data?.reduce((sum, e) => sum + parseFloat(e.amount || '0'), 0) ?? 0;

  const stats = [
    {
      title: 'مبيعات اليوم',
      value: `${fmt(todayReport?.total_revenue ?? 0)} ₪`,
      subtext: `${todayReport?.sales_count ?? 0} فاتورة`,
      icon: DollarSign,
    ...
    {
      title: 'مشتريات اليوم',
      value: `${fmt(todayPurchasesTotal)} ₪`,
      subtext: `${todayPurchases?.data?.length ?? 0} فاتورة شراء`,
      icon: ShoppingBag,
      color: 'text-blue-400',
    ...
    {
      title: 'مصروفات اليوم',
      value: `${fmt(todayExpensesTotal)} ₪`,
      subtext: `${todayExpenses?.data?.length ?? 0} عملية`,
      icon: CreditCard,
      color: 'text-rose-400',
    ...
    },
    {
      ...
      value: `${fmt(todayReport?.net_profit ?? 0)} ₪`,
      subtext: `هامش ${todayReport?.net_margin_percent ?? 0}%`,
      icon: TrendingUp,
    ...
    {
      title: 'ديون العملاء',
      value: `${fmt(totalCustomerDebt)} ₪`,
      subtext: `${customersData?.data?.filter(c => parseFloat(c.current_balance || '0') > 0).length ?? 0} عميل عليه رصيد`,
      icon: Users,
      color: 'text-sky-400',
    ...
    },
    {
      ...
      value: `${fmt(totalSupplierPayables)} ₪`,
      subtext: `${suppliersData?.data?.filter(s => parseFloat(s.current_balance || '0') > 0).length ?? 0} مورد لديه رصيد`,
      icon: Truck,
    ...
    },
    {
      ...
      value: `${productMetrics?.total_products ?? 0} صنف`,
      subtext: productMetrics?.low_stock_count && productMetrics.low_stock_count > 0
        ? `⚠ ${productMetrics.low_stock_count} صنف مخزونه منخفض`
      ...
      <div className="relative overflow-hidden rounded-3xl bg-gradient-to-r from-slate-900 via-slate-850 to-slate-900 border border-slate-800 p-6 md:p-8 shadow-xl">
        ...
        <div className="relative z-10 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3">
            <Link to="/pos">
              <Button size="lg" rightIcon={<ShoppingCart className="w-4 h-4" />}>
                فتح نقطة البيع
              </Button>
            </Link>
          </div>
            ...
            return (
              <Link key={idx} to={action.to}>
                <button
                  className={
                    action.primary
                      ? 'flex items-center gap-2 px-5 py-3 rounded-xl font-bold text-sm bg-brand-500 hover:bg-brand-600 text-slate-950 transition-colors shadow-lg shadow-brand-500/20'
                      : 'flex items-center gap-2 px-4 py-2.5 rounded-xl font-semibold text-sm bg-slate-800 hover:bg-slate-700 text-slate-200 border border-slate-700 transition-colors'
                  }
                >
