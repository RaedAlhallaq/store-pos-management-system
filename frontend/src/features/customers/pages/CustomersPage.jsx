...
export function CustomersPage() {
  ...
  const handleOpenEdit = (customer) => {
    ...
    setFormCreditLimit(String(customer.credit_limit || '0.00'));
  ...
  const totalOutstandingDebt = customers.reduce(
    (sum, c) => sum + Math.max(0, Number(c.current_balance)),
    0,
  );
    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              {isLoading ? (
                ...
                <p className="text-xl font-bold text-slate-100 font-mono mt-1">{meta?.total ?? 0}</p>
              )}
            </div>
        ...
        <div className="p-4 rounded-2xl bg-slate-900/80 border border-slate-800">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-xs font-semibold text-slate-400">عملاء نشطون</p>
              {isLoading ? (
                <div className="h-7 w-16 mt-1.5 bg-slate-800 rounded-lg animate-pulse" />
              ) : (
                <p className="text-xl font-bold text-emerald-400 font-mono mt-1">{activeCount}</p>
              )}
            </div>
        ...
        <div className="p-4 rounded-2xl bg-slate-900/80 border border-slate-800">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-xs font-semibold text-slate-400">عملاء مدينون</p>
              {isLoading ? (
                <div className="h-7 w-16 mt-1.5 bg-slate-800 rounded-lg animate-pulse" />
              ) : (
                <p className="text-xl font-bold text-rose-400 font-mono mt-1">{debtCount}</p>
              )}
            </div>
        ...
        <div className="p-4 rounded-2xl bg-slate-900/80 border border-slate-800">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-xs font-semibold text-slate-400">إجمالي المديونيات</p>
              {isLoading ? (
                <div className="h-7 w-20 mt-1.5 bg-slate-800 rounded-lg animate-pulse" />
              ) : (
                <p className="text-xl font-bold text-amber-400 font-mono mt-1">{totalOutstandingDebt.toFixed(2)} ₪</p>
              )}
            </div>
      ...
      <div className="bg-slate-900/60 p-3 sm:p-4 rounded-2xl border border-slate-800 space-y-3">
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3">
          <div className="relative flex-1 sm:max-w-xs">
            ...
            <input
              type="text"
              placeholder="بحث بالاسم، الهاتف، أو الضريبي..."
              value={search}
              ...
              <span key={label} className="inline-flex items-center gap-1 bg-brand-500/10 text-brand-400 text-[11px] font-medium px-2 py-0.5 rounded-lg border border-brand-500/20">
                ...
                <button
                  ...
                  onClick={() => {
                    ...
                    if (label.startsWith('مدين')) setHasDebt(undefined);
                    setPage(1);
                  }}
      ...
      {isLoading ? (
            ...
            <table className="w-full text-right text-xs">
              <thead>
                <tr className="bg-slate-950/80 border-b border-slate-800">
                  {['اسم العميل', 'الهاتف', 'الرصيد', 'الحد الائتماني', 'الحالة', 'إجراءات'].map((h) => (
        ...
        <EmptyState
          ...
          description={hasActiveFilters ? 'جرّب تغيير عوامل التصفية أو البحث بكلمات مختلفة' : 'يمكنك إضافة عملاء لتسجيل الفواتير الآجلة ومتابعة الأرصدة'}
          actionLabel={hasActiveFilters ? 'مسح التصفية' : 'إضافة عميل جديد'}
        ...
        <div className="bg-slate-900/70 border border-slate-800 rounded-2xl overflow-hidden">
          ...
          <div className="lg:hidden divide-y divide-slate-800/60">
            {customers.map((customer) => {
              ...
              const balancePercent = limit > 0 ? Math.min(100, (balance / limit) * 100) : 0;
              ...
              return (
                <div key={customer.id} className="p-4 space-y-3">
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2 min-w-0">
                      ...
                      <div className="min-w-0">
                        <span className="font-bold text-slate-100 text-sm block truncate">{customer.name}</span>
                        {customer.tax_number && (
                          <span className="text-[10px] text-slate-500 font-mono flex items-center gap-1">
                            <Hash className="w-3 h-3" />{customer.tax_number}
                          </span>
                        )}
                      </div>
                    </div>
                  ...
                  <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-slate-400">
                    {customer.phone && (
                      <span className="flex items-center gap-1 font-mono"><Phone className="w-3 h-3 text-slate-500" />{customer.phone}</span>
                    )}
                    {customer.email && (
                      <span className="flex items-center gap-1 truncate"><Mail className="w-3 h-3 text-slate-500" />{customer.email}</span>
                    )}
                    {customer.address && (
                      <span className="flex items-center gap-1 truncate"><MapPin className="w-3 h-3 text-slate-500" />{customer.address}</span>
                    )}
                  </div>
                  ...
                  <div className="space-y-1">
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-1.5">
                        <span className={`font-mono text-sm font-bold ${balance > 0 ? 'text-amber-400' : balance < 0 ? 'text-emerald-400' : 'text-slate-400'}`}>
                          {balance.toFixed(2)} ₪
                        </span>
                        {isOverLimit && <Badge variant="danger" size="sm">تجاوز</Badge>}
                      </div>
                    </div>
                    {limit > 0 && (
                      <div className="w-full bg-slate-800 rounded-full h-1.5 overflow-hidden">
                        <div
                          className={`h-full rounded-full transition-all ${balancePercent >= 100 ? 'bg-rose-500' : balancePercent >= 70 ? 'bg-amber-500' : 'bg-brand-500'}`}
                    ...
                    {limit > 0 && (
                      <div className="flex justify-between text-[10px] text-slate-500">
                        <span>الحد: {limit.toFixed(2)} ₪</span>
                  ...
                  <div className="flex items-center gap-2 pt-1 border-t border-slate-800">
                    {balance > 0 && (
                      <Button
                        variant="ghost"
                        size="sm"
                        onClick={() => { setPayingCustomer(customer); setPaymentAmount(balance.toFixed(2)); }}
                        className="text-emerald-400 hover:bg-emerald-500/10 gap-1 flex-1 justify-center"
                      >
                    ...
                    <tr key={customer.id} className="hover:bg-slate-800/30 transition-colors">
                      <td className="py-3.5 px-4">
                        <div className="flex items-center gap-2.5">
                          ...
                          <div>
                            <span className="font-bold text-slate-100 text-sm block">{customer.name}</span>
                            {customer.tax_number && (
                              <span className="text-[10px] text-slate-500 font-mono">ضريبي: {customer.tax_number}</span>
                            )}
                          </div>
                        </div>
                      </td>
                      ...
                      <td className="py-3.5 px-4 max-w-[200px]">
                        <div className="space-y-0.5">
                          ...
                          {!customer.email && !customer.address && <span className="text-slate-600">—</span>}
                        </div>
                      </td>
                      <td className="py-3.5 px-4 font-mono">
                        <div className="flex items-center gap-2">
                          <span className={`font-bold text-sm ${balance > 0 ? 'text-amber-400' : balance < 0 ? 'text-emerald-400' : 'text-slate-400'}`}>
                            {balance.toFixed(2)} ₪
                          </span>
                          {isOverLimit && <Badge variant="danger" size="sm">تجاوز السقف</Badge>}
                        </div>
                      </td>
                      <td className="py-3.5 px-4 font-mono text-slate-400">
                        {limit > 0 ? `${limit.toFixed(2)} ₪` : <span className="text-slate-600">غير محدد</span>}
                      </td>
                      <td className="py-3.5 px-4">
                        {customer.is_active ? (
                          <Badge variant="success" size="sm">نشط</Badge>
          ...
          <form onSubmit={handleSaveCustomer} className="space-y-4">
            ...
            {editingCustomer && (
              <div className="p-3 rounded-xl bg-slate-950 border border-slate-800 flex items-center justify-between">
                ...
                <span className={`font-mono text-sm font-bold ${Number(editingCustomer.current_balance) > 0 ? 'text-amber-400' : 'text-slate-300'}`}>
                  {Number(editingCustomer.current_balance).toFixed(2)} ₪
                </span>
              </div>
            )}

            <div className="flex items-center justify-end gap-3 pt-3 border-t border-slate-800">
              <Button type="button" variant="outline" onClick={() => setIsCustomerModalOpen(false)} disabled={isSaving}>
                إلغاء
              </Button>
              <Button type="submit" isLoading={isSaving} rightIcon={<Check className="w-4 h-4" />}>
        ...
        >
          <form onSubmit={handleConfirmPayment} className="space-y-4">
            ...
            <div className="p-3 rounded-xl bg-slate-950 border border-slate-800 space-y-2">
              ...
              {Number(payingCustomer.credit_limit) > 0 && (
                <div className="flex items-center justify-between text-xs">
                  <span className="text-slate-400">الحد الائتماني</span>
                  <span className="font-mono text-slate-300">{Number(payingCustomer.credit_limit).toFixed(2)} ₪</span>
                </div>
              )}
            </div>
            ...
            <Input
              ...
              placeholder="مثال: دفعة من حساب الفاتورة"
              value={paymentNotes}
            ...
            <div className="flex items-center gap-2">
              ...
              {[
                ...
                <button
                  key={label}
                  type="button"
                  onClick={() => setPaymentAmount((Number(payingCustomer.current_balance) * fraction).toFixed(2))}
            ...
            <div className="flex items-center justify-end gap-3 pt-3 border-t border-slate-800">
              <Button type="button" variant="outline" onClick={() => setPayingCustomer(null)} disabled={isPaying}>
                إلغاء
              </Button>
              <Button
                type="submit"
