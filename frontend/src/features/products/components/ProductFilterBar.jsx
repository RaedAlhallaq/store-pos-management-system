  ...
  return (
    <div className="space-y-3 bg-slate-900/60 p-4 rounded-2xl border border-slate-800">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
        ...
        <div>
          <select
            ...
            onChange={(e) => onFilterChange({ category_id: e.target.value, page: 1 })}
      ...
      <div className="flex flex-wrap items-center gap-2 pt-1 border-t border-slate-800/80 text-xs">
        ...
        <button
          type="button"
          onClick={() => onFilterChange({ stock_status: 'low', page: 1 })}
          className={`px-3 py-1 rounded-lg font-semibold flex items-center gap-1.5 transition-colors ${
            filters.stock_status === 'low'
              ? 'bg-amber-500 text-slate-950 font-bold'
              : 'bg-slate-800 text-amber-400/90 hover:bg-slate-750'
          }`}
        >
          <AlertTriangle className="w-3.5 h-3.5" />
          <span>نواقص وحد الطلب</span>
        </button>

        <button
          type="button"
          onClick={() => onFilterChange({ stock_status: 'out', page: 1 })}
          className={`px-3 py-1 rounded-lg font-semibold flex items-center gap-1.5 transition-colors ${
        ...
        <button
          type="button"
          onClick={() => onFilterChange({ stock_status: 'in_stock', page: 1 })}
          className={`px-3 py-1 rounded-lg font-semibold flex items-center gap-1.5 transition-colors ${
