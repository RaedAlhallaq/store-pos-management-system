...
export function PosPage() {
  ...
  const refreshProducts = useCallback(async () => {
    try { setProductsData(await productsApi.getProducts({ per_page: 250 })); }
    catch { toast.error('فشل تحميل المنتجات'); }
    finally { setIsProductsLoading(false); }
  }, []);
  const refreshCategories = useCallback(async () => {
    try { setCategories(await productsApi.getCategories()); }
    catch { setCategories([]); }
  }, []);
  const refreshCustomers = useCallback(async () => {
    try { setCustomers(await customersApi.getQuickList()); }
    catch { setCustomers([]); }
  }, []);
  ...
  }, [refreshProducts, refreshCategories, refreshCustomers]);
  ...
  const playBeep = () => {
    ...
    } catch { /* Audio can be blocked before a user gesture. */ }
  };
    ...
    setCartItems((current) => {
      ...
      if (existing) {
        ...
        return current.map((item) => item.product.id === product.id
          ? { ...item, quantity, subtotal: unitPrice * quantity - item.discount_amount }
          : item);
      }
      return [...current, {
        product,
  ...
  const updateQuantity = (productId, quantity) => {
    ...
    setCartItems((current) => current.map((item) => {
      ...
      return { ...item, quantity, subtotal: item.unit_price * quantity - item.discount_amount };
    }));
  };
  ...
  const checkout = async (paymentData) => {
    ...
    try {
      ...
      toast.error(error.response?.data?.message || 'فشلت معالجة الفاتورة');
    } finally {
  ...
  useEffect(() => {
    const keydown = (event) => {
      if (event.key === 'F2') {
        ...
        if (cartItems.length) setIsPaymentModalOpen(true);
      }
    };
  ...
  const grandTotal = Math.max(0, cartItems.reduce((sum, item) => sum + item.subtotal, 0) - overallDiscount);
    ...
    <div className="h-[calc(100vh-5.5rem)] flex flex-col lg:flex-row gap-4 overflow-hidden">
      ...
      <div className="w-full lg:w-96 xl:w-[420px] h-full flex flex-col">
        <PosCart
          ...
          onRemoveItem={(id) => setCartItems((current) => current.filter((item) => item.product.id !== id))}
      ...
      {isReceiptModalOpen && (
        <ReceiptModal
          ...
          onNewSale={() => { setLastCompletedSale(null); setIsReceiptModalOpen(false); }}
        />
      )}
      {isQuickCustomerOpen && (
        <QuickCustomerModal
          ...
          onCustomerCreated={async (customer) => { await refreshCustomers(); setSelectedCustomer(customer); }}
        />
      )}
      {isHeldOrdersOpen && (
        <HeldOrdersModal
      ...
      {isClearConfirmOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="w-full max-w-sm bg-slate-900 border border-slate-700 rounded-2xl shadow-2xl p-6 space-y-4">
            <h3 className="font-bold text-slate-100 text-sm">مسح جميع عناصر السلة؟</h3>
            <p className="text-xs text-slate-400">سيتم حذف جميع الأصناف والخصومات من الفاتورة الحالية.</p>
            <div className="flex items-center justify-end gap-2 pt-2">
              <Button variant="outline" size="sm" onClick={() => setIsClearConfirmOpen(false)}>تراجع</Button>
              <Button size="sm" onClick={confirmClearCart} className="bg-rose-600 hover:bg-rose-700 text-white">مسح السلة</Button>
            </div>
          </div>
        </div>
      )}
