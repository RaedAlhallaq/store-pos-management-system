...
import { PauseCircle, PlayCircle, Trash2, Clock, User, ShoppingBag } from 'lucide-react';
import type { HeldOrder } from '../types/posTypes';
...
export const HeldOrdersModal: React.FC<HeldOrdersModalProps> = ({
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={onClose}
      title="الفواتير المعلقة مؤقتاً"
      subtitle={`يوجد (${heldOrders.length}) فواتير معلقة بانتظار الاستئناف`}
      maxWidth="md"
    >
      <div className="space-y-3">
        {heldOrders.length === 0 ? (
          <div className="text-center py-8 text-slate-500">
            <PauseCircle className="w-10 h-10 mx-auto mb-2 text-slate-600" />
